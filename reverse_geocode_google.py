...
def column_map(ws) -> dict[str, int]:
    ...
    missing = {"orderid", "longitude", "latitude"} - headers.keys()
    if missing:
...
def process_google(input_xlsx: Path, output_xlsx: Path, key: str, delay: float = 0.08) -> dict:
    ...
    log.append(["baris_sumber", "order_id", "latitude", "longitude", "status", "alamat_google", "catatan"])
    ...
    for row_no, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        bb_id, lng_raw, lat_raw = row[cols["orderid"]-1], row[cols["longitude"]-1], row[cols["latitude"]-1]
        try:
        ...
        result.append([bb_id, lat, lng, *address_tuple]); log.append([row_no, bb_id, lat, lng, status, google_address, "" if complete else "Periksa hasil Google sebelum digunakan sebagai alamat final."])
        counts[status] += 1
    ...
    for column, width in {"A":18,"B":14,"C":14,"D":20,"E":20,"F":20,"G":20}.items(): result.column_dimensions[column].width = width
